...
export default function RegisterStudent() {
  ...
  function onPhoto(file: File | null) { setPhoto(file); setPhotoPreview(file ? URL.createObjectURL(file) : '') }

  async function submit() {
    if (!course || !branch || !amountValid || !bankRef.trim()) { toast.error(t('reg.minFirst')); return }
    setSaving(true)
    try {
      const { regNo } = await busy.run(
        () => registerStudent({
          branchId: branch.id, branchName: branch.name, fullName: fullName.trim(), gender,
          phone: phone.trim(), nida: nida.trim(), tin: tin.trim(), residence: residence.trim(),
          nextOfKin: { name: nokName.trim(), relationship: nokRel.trim(), residence: '', phone: nokPhone.trim() },
          course, firstPayment: { amount: amt, period: t('reg.firstPayment'), bankRef: bankRef.trim() },
          createdBy: user?.uid ?? '',
        }, photo),
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="flex items-center mt-6 max-w-[720px]">
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center flex-1 last:flex-none">
              ...
              {i < STEPS.length - 1 && <div className={`h-[2px] flex-1 mx-3 rounded ${i < step ? 'bg-primary' : 'bg-surface-soft'}`} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 px-6 sm:px-8 py-6">
        <AnimatePresence mode="wait" custom={dir}>
          <motion.div key={step} custom={dir} variants={stepVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.22, ease: 'easeOut' }}>
                  ...
                  <div className="shrink-0">
                    <label className="cursor-pointer block w-[120px]">
                      <div className="w-[120px] h-[150px] rounded-[14px] bg-surface-soft grid place-items-center overflow-hidden text-ink-muted">
                        {photoPreview ? <img src={photoPreview} alt="" className="w-full h-full object-cover" /> : (<div className="text-center"><Camera size={24} /><div className="text-[10px] font-semibold mt-1">{t('reg.photoHint')}</div></div>)}
                      </div>
              ...
              <div className="bg-surface rounded-[18px] shadow-soft p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-[720px]">
                  <Field label={t('reg.selectCourse')}>
                    <Dropdown value={courseId} onChange={setCourseId} placeholder={t('reg.selectCoursePh')}
                      options={courses.map((c) => ({ value: c.id, label: c.name, hint: `${durationLabel(c)} · TSh ${formatMoney(c.price)} · ${courseCode(c)}` }))} />
                  </Field>
                ...
                {course && (<div className="mt-4 inline-flex items-center gap-2 bg-surface-soft rounded-md px-4 py-3 text-[13px]"><span className="font-semibold text-ink-secondary">{t('reg.totalFee')}:</span><span className="font-extrabold tabular-nums">TSh {formatMoney(course.price)}</span></div>)}
              </div>
            )}

            {step === 2 && course && (
              <div className="bg-surface rounded-[18px] shadow-soft p-6 max-w-[720px]">
                <div className="text-[13px] font-extrabold mb-1">{t('reg.firstPayment')}</div>
                <div className="text-[11px] text-ink-muted font-semibold mb-4">{t('reg.bankRefHint')} · {t('reg.minFirst')}</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label={t('reg.amountPaid')}>
                    <input type="number" min={MIN_FIRST_PAYMENT} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="50,000" className={inputClass} />
                    {amount !== '' && !amountValid && <div className="text-[10.5px] font-semibold text-danger mt-1.5">{t('reg.minFirst')}</div>}
                  </Field>
                  <Field label={t('reg.bankRef')}><input value={bankRef} onChange={(e) => setBankRef(e.target.value)} className={inputClass} /></Field>
                </div>
                {amt > 0 && (
                  <div className="mt-4 flex items-center gap-2 text-[13px]">
                    <span className="font-semibold text-ink-secondary">{t('reg.balanceAfterPay')}:</span>
            ...
            {step === 3 && course && branch && (
              <div className="bg-surface rounded-[18px] shadow-soft p-6 max-w-[820px]">
                ...
                <div className="flex items-center gap-4 pb-5 mb-5 border-b border-hair">
                  <div className="w-[64px] h-[80px] rounded-[12px] bg-surface-soft overflow-hidden grid place-items-center text-ink-muted shrink-0">{photoPreview ? <img src={photoPreview} alt="" className="w-full h-full object-cover" /> : <Camera size={20} />}</div>
                  <div><div className="font-extrabold text-[16px]">{fullName}</div><div className="text-[12px] text-ink-muted font-semibold mt-0.5">{t('reg.regNoPreview')}: <span className="tabular-nums">HKM/VTC/{courseCode(course)}/••••</span></div></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3.5 text-[12.5px]">
                  ...
                  <Review label={t('reg.section.course')} value={`${course.name} · ${durationLabel(course)}`} />
                  <Review label={t('reg.totalFee')} value={`TSh ${formatMoney(course.price)}`} />
                  <Review label={t('reg.amountPaid')} value={`TSh ${formatMoney(amt)}`} />
                  <Review label={t('reg.bankRef')} value={bankRef} />
                  <Review label={t('reg.balanceAfterPay')} value={`TSh ${formatMoney(balanceAfter)}`} />
                </div>
              </div>
            )}
      ...
      <div className="px-6 sm:px-8 py-4 flex items-center justify-between bg-canvas sticky bottom-0">
        <button type="button" onClick={() => { setDir(-1); setStep((s) => Math.max(0, s - 1)) }} disabled={step === 0 || saving} className="inline-flex items-center gap-1.5 text-[13px] font-bold text-ink-secondary hover:text-ink px-4 py-3 rounded-md disabled:opacity-40 transition"><ArrowLeft size={16} />{t('reg.backStep')}</button>
        <div className="text-[11.5px] font-bold text-ink-muted hidden sm:block">{t('reg.stepLabel')} {step + 1} {t('reg.of')} {STEPS.length}</div>
        {step < 3 ? (
          <button type="button" onClick={() => { if (canNext) { setDir(1); setStep((s) => Math.min(3, s + 1)) } }} disabled={!canNext} className="inline-flex items-center gap-2 text-sm font-bold text-on-primary bg-primary rounded-md min-h-[46px] px-6 hover:opacity-90 transition disabled:opacity-40">{t('reg.next')}<ArrowRight size={16} /></button>
        ) : (
          <button type="button" onClick={submit} disabled={saving} className="inline-flex items-center justify-center gap-2 text-sm font-bold text-on-primary bg-primary rounded-md min-h-[46px] min-w-[180px] px-6 hover:opacity-90 transition disabled:opacity-60">{saving ? <Wave className="h-4 w-10" /> : t('reg.submit')}</button>
        )}
      </div>
    </div>
  )
...
function Review({ label, value }: { label: string; value: string }) {
  return (<div><div className="text-[10.5px] font-bold text-ink-muted">{label}</div><div className="font-semibold text-ink mt-0.5 break-words">{value}</div></div>)
}