...
import { ArrowLeft, PencilSimple, Check, X, Prohibit, Trash, Warning, Plus, Printer } from '@phosphor-icons/react'
...
import { useStudentDetail, updateStudent, dropStudent, deleteStudent, recordPayment } from '../../lib/services/students'
...
export default function StudentDetail() {
  ...
  useEffect(() => {
    if (student) {
      setForm({
        phone: student.phone ?? '', nida: student.nida ?? '', tin: student.tin ?? '',
  ...
  function printPaymentReceipt(pay: { amount: number; period: string; bankRef: string; createdAt: number }) {
    ...
    const ok = printReceipt({
      receiptNo: pay.bankRef || '—',
      date: formatDate(pay.createdAt),
      studentName: student.fullName,
      regNo: student.regNo,
      course: enrollment?.courseName ?? '—',
      amount: `TSh ${formatMoney(pay.amount)}`,
      period: pay.period || '—',
      balance: `TSh ${formatMoney(enrollment?.balance ?? 0)}`,
      labels: {
        title: t('receipt.title'), no: t('receipt.no'), date: t('receipt.date'),
        student: t('receipt.student'), reg: t('receipt.reg'), course: t('receipt.course'),
        amount: t('receipt.amount'), for: t('receipt.for'), balance: t('receipt.balance'),
        received: t('receipt.received'), thanks: t('receipt.thanks'),
      },
    })
  ...
  async function confirmAction() {
    ...
    setBusy(true)
    try {
      if (pending === 'drop' && enrollment) {
        await dropStudent(enrollment.id)
        toast.success(t('detail.dropped'))
        setPending(null)
  ...
  if (loading) {
    return <div className="p-6 sm:p-7"><Skeleton className="h-5 w-32 mb-6" /><Skeleton className="h-40 w-full rounded-[18px]" /></div>
  }
  if (!student) {
    return (
      <div className="p-6 sm:p-7">
        <Link to="/students" className="inline-flex items-center gap-1.5 text-[12px] font-bold text-ink-secondary hover:text-ink mb-6"><ArrowLeft size={15} /> {t('detail.back')}</Link>
        <div className="py-16 text-center text-[14px] font-bold text-ink-muted">{t('detail.notFound')}</div>
      </div>
    )
  }

  const dropped = enrollment?.status === 'dropped'
  const inst = enrollment?.installmentAmount && enrollment.installmentAmount > 0 ? enrollment.installmentAmount : (enrollment?.balance ?? 0)
  const bal = enrollment?.balance ?? 0
  const payCount = inst > 0 ? Math.max(1, Math.ceil(bal / inst)) : 1
  const payOptions = enrollment
    ? Array.from({ length: payCount }, (_, i) => {
        const k = i + 1
        const amt = Math.min(k * inst, bal)
        const isFull = amt >= bal
        const label = isFull
          ? `${t('detail.fullRemaining')} — TSh ${formatMoney(amt)}`
          : k === 1
          ? `${t('detail.nextInstallment')} — TSh ${formatMoney(amt)}`
          : `${t('reg.installmentsWord')} ×${k} — TSh ${formatMoney(amt)}`
        return { value: String(k), label, amount: amt, isFull }
      })
    : []
  const selectedPay = payOptions.find((o) => o.value === payK) ?? payOptions[0]
  const payAmount = selectedPay?.amount ?? 0

  async function recordPay() {
    if (!enrollment || !selectedPay) return
    setRecording(true)
    try {
      const period = selectedPay.isFull
        ? t('reg.fullPayment')
        : Number(payK) === 1
        ? t('reg.installmentWord')
        : `${t('reg.installmentsWord')} ×${payK}`
      await busyOverlay.run(() => recordPayment(enrollment.id, payAmount, period, payRef.trim()), { message: t('busy.recording'), timeoutMs: 20000 })
      toast.success(t('detail.recordSuccess'))
      setShowPay(false)
      setPayRef('')
      setPayK('1')
    } catch (err) {
      toast.error(err instanceof TimeoutError ? t('err.timeout') : t('detail.recordError'))
    } finally {
      setRecording(false)
    }
  }
    ...
    <div className="p-6 sm:p-7 max-w-[900px]">
      ...
      <div className="bg-surface rounded-[18px] shadow-soft p-5 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-extrabold text-[18px] truncate">{student.fullName}</div>
          <div className="text-[12px] text-ink-muted font-semibold mt-0.5 tabular-nums">{student.regNo}</div>
        </div>
        {enrollment && (
          dropped
            ? <StatusBadge kind="dropped" label={t('detail.statusDropped')} />
            : <StatusBadge kind={enrollment.balance <= 0 ? 'paid' : 'due'} label={enrollment.balance <= 0 ? t('dash.paid') : t('dash.balanceDue')} />
        )}
      </div>
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <div className="mt-5 pt-5 border-t border-hair">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-[560px]">
                    <div>
                      <label className="block text-[12px] font-bold mb-[7px] text-ink">{t('reg.chooseInstallment')}</label>
                      <Dropdown value={payK} onChange={setPayK} options={payOptions.map((o) => ({ value: o.value, label: o.label }))} />
                    </div>
                    <div>
                      <label className="block text-[12px] font-bold mb-[7px] text-ink">{t('reg.bankRef')}</label>
                      <input value={payRef} onChange={(e) => setPayRef(e.target.value)} className={inputClass} />
                    </div>
                  </div>
      ...
      <div className="bg-surface rounded-[18px] shadow-soft p-5 mt-5">
        ...
        {payments.length === 0 ? (
          <div className="text-[12.5px] text-ink-muted font-semibold py-2">{t('detail.noPayments')}</div>
        ) : (
      ...
      <div className="mt-5 rounded-[18px] p-5 bg-[rgba(201,71,71,0.04)]">
        ...
        {pending ? (
          <div className="mt-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPending(null)} disabled={busy} className="bg-surface hover:bg-surface-soft rounded-[20px] px-4 py-2 text-[12px] font-bold text-ink-secondary transition shadow-soft">{t('detail.cancel')}</button>
              <button onClick={confirmAction} disabled={busy} className="inline-flex items-center justify-center gap-1.5 bg-danger text-white rounded-[20px] px-4 py-2 min-w-[120px] text-[12px] font-bold hover:brightness-95 transition disabled:opacity-60">{busy ? <Wave className="h-3.5 w-8 text-white" /> : t('detail.confirm')}</button>
            </div>
          </div>
        ) : (
          <div className="flex flex-wrap gap-2 mt-3">
            {enrollment && !dropped && (
              <button onClick={() => setPending('drop')} className="inline-flex items-center gap-1.5 bg-surface hover:bg-surface-soft rounded-[20px] px-4 py-2 text-[12px] font-bold text-ink-secondary shadow-soft transition"><Prohibit size={14} /> {t('detail.drop')}</button>
            )}
            {isAdmin && (
              <button onClick={() => setPending('remove')} className="inline-flex items-center gap-1.5 bg-surface hover:bg-surface-soft rounded-[20px] px-4 py-2 text-[12px] font-bold text-danger shadow-soft transition"><Trash size={14} /> {t('detail.remove')}</button>
            )}
          </div>
        )}
      </div>
...
function Row({ label, value, edit, onChange }: { label: string; value: string; edit?: boolean; onChange?: (v: string) => void }) {
  return (
    <div>
      <div className="text-[10.5px] font-bold text-ink-muted mb-1">{label}</div>
      {edit && onChange ? <input value={value} onChange={(e) => onChange(e.target.value)} className={inputClass} /> : <div className="font-semibold text-[13px] text-ink break-words">{value || '—'}</div>}
    </div>
  )
}